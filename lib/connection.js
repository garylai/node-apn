...
Connection.prototype.bufferNotification = function (notification) {
	if (notification.retryLimit === 0) {
		...
		this.emit("transmissionError", Errors.retryLimitExceeded, notification.notification.id);
		return;
	}
...
Connection.prototype.rejectBuffer = function (errCode) {
	while(this.notificationBuffer.length > 0) {
		...
		this.emit("transmissionError", errCode, notification.notification.id);
	}
};
...
Connection.prototype.prepareNotification = function (notification, device) {
	...
	if (recipient === undefined) {
		util.setImmediate(function () {
			...
			this.emit("transmissionError", Errors.missingDeviceToken, notification.id);
		}.bind(this));
		return;
	}
	...
	if (!(recipient instanceof Device)) {
		try {
			...
			util.setImmediate(function () {
				...
				this.emit("transmissionError", Errors.invalidToken, notification.id);
			}.bind(this));
			return;
		}
	}
...
Connection.prototype.handleTransmissionError = function (socket, data) {
	if (data[0] === 8) {
		...
		this.emit("transmissionError", errorCode, identifier);
...
Connection.prototype.transmitNotification = function(socket, notification) {
	...
	data.writeUInt32BE(notification.notification.id, position); position += 4;
    ...
    this.emit("transmitted", notification.notification.id);
    return result;
};
...
Connection.prototype.validNotification = function (notification, recipient) {
	...
	if (messageLength > maxLength) {
		util.setImmediate(function () {
			...
			this.emit("transmissionError", Errors.invalidPayloadSize, notification.id);
		}.bind(this));
		return false;
	}
	return true;
...
Connection.prototype.pushNotification = function (notification, recipient) {
	if (this.terminated) {
		this.emit("transmissionError", Errors.connectionTerminated, notification.id);
		return false;
	}
